...
class SegmentSchedule:
    ...
    def __post_init__(self) -> None:
        ...
        assert len(self.segment_rel_times_s) == len(
            self.commands
        ), "Schedule/commands mismatch."

        for command in self.commands:
            traj = (
                command.synchronized_command.arm_command.arm_joint_move_command.trajectory
            )
            assert traj.HasField(
                "reference_time"
            ), "Segment must have a (local) reference_time."
    ...
    def compute_send_local_time_s(self, idx: int, send_early_s: float) -> float:
        ...
        assert (
            0 <= idx < len(self.segment_rel_times_s)
        ), f"Segment index {idx} is out of range."
    ...
    def slide_segment_if_late(
        ...
        if segment_start_local_s <= earliest_start_local_s:
            delta_s = max(
                earliest_start_local_s - segment_start_local_s, self.MIN_BUMP_S
            )

            new_ref_local_s = (
                self.ref_local_time_s + delta_s
            )  # New shared reference time
