...
class SpotCommander(Node):
    ...
    def resync_and_log(self) -> None:
        ...
        self.get_logger().info(
            f"Maximum observed round trip time: {max_round_trip_s} seconds."
        )

        clock_skew_s = self.time_sync.get_robot_clock_skew_s()
        self.get_logger().info(
            f"Current robot clock skew from local: {clock_skew_s} seconds."
        )
    ...
    def take_control_and_power_on(
        self, resource: str = "body", force: bool = False
    ) -> bool:
        ...
        self.get_logger().info(
            "Spot powered on." if power_success else "Spot power on failed."
        )
    ...
    def stand_up(
        ...
        control_params: Optional[BodyControlParams] = None,
    ) -> bool:
